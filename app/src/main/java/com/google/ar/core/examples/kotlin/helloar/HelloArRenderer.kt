...
class HelloArRenderer(val activity: HelloArActivity) :
    ...
    private fun handleTap(frame: Frame, camera: Camera) {
        ...
        val hitResultList =
            if (activity.instantPlacementSettings.isInstantPlacementEnabled) {
                frame.hitTestInstantPlacement(tap.x + count, tap.y, APPROXIMATE_DISTANCE_METERS)
            } else {
                frame.hitTest(tap)
            }
   ...
   /* fun mapAnchors(places:List<com.google.codelabs.findnearbyplacesar.model.Place>) {
        val session = session ?: return
        // ... your list of places ...

        val cameraAltitude = if (frame?.camera?.trackingState == TrackingState.TRACKING) {
            frame!!.camera.pose.ty() // This gives the Y (upward) translation component of the camera pose
        } else {
        ...
        for (place in places) {
            ...
            if (location != null) {
                ...
                // Use latitude, longitude, and camera altitude to calculate the translation
                val translation = calculateTranslation(latitude, longitude, cameraAltitude)

                // Create a new anchor with the calculated translation
                val newAnchor = try {
                ...
                // Check if the anchor was successfully created
                if (newAnchor != null) {

                    // Add the new anchor with the associated trackable to your wrappedAnchors list
                    wrappedAnchors.add(WrappedAnchor(newAnchor, null))
                }
            }
        }
    }
    ...
    // Function to calculate translation based on latitude, longitude, and camera altitude
    private fun calculateTranslation(latitude: Double, longitude: Double, cameraAltitude: Float): FloatArray {
        // Implement your logic to calculate translation based on lat, lng, and camera altitude
        // For simplicity, let's assume a direct mapping for now
        return floatArrayOf(longitude.toFloat(), cameraAltitude, latitude.toFloat())
    }
*/
    ...
    private fun makeMoveToFixed() {
        ...
        Log.d(TAG, "makeMoveToFixed: ${wrappedAnchors.size}")
        // Get the last created anchor.
        val lastWrappedAnchor = wrappedAnchors.last()
        val lastAnchor = lastWrappedAnchor.anchor
        val currentPose = lastAnchor.pose

        // Calculate the new position relative to the camera.
        val updatedTranslation = addAnchorAtFixedDistance(frame, session, 10f)

        // Create a new pose with the updated translation.
        val updatedPose = Pose(updatedTranslation, currentPose.rotationQuaternion)

        // Create a new anchor with the updated pose and attach it to the session.
        if (session.hasTrackingPlane() && updatedTranslation[0] != 0f && updatedTranslation[1] != 0f && updatedTranslation[2] != 0f) {
            val updatedAnchor = session.createAnchor(updatedPose)

            wrappedAnchors.add(WrappedAnchor(updatedAnchor, lastWrappedAnchor.trackable))
        }

        Log.d(TAG, "makeMoveToFixed: (${updatedTranslation.asList()})")
    }

    // Function to add a new anchor at a fixed distance in front of the camera
    private fun addAnchorAtFixedDistance(
        frame: Frame,
        session: Session,
        dist: Float = 2f
    ): FloatArray {
        ...
        if (frame.camera.trackingState == TrackingState.TRACKING) {
            // Create a new anchor at a fixed distance in front of the camera
            val newAnchor = try {
                // Create an anchor 1 meter in front of the camera
                session.createAnchor(
                    frame.camera.pose.compose(Pose.makeTranslation(0f, 0f, dist))
                        .extractTranslation()
                )
            } catch (e: Exception) {
                Log.e(TAG, "Error creating anchor: ${e.message}")
            ...
            if (newAnchor != null) {
                ...
                return newAnchor.pose.extractTranslation().translation
            } else {
...
private fun addPlaces(anchorNode: MutableList<WrappedAnchor>) {
    ...
    for (place in places) {
        ...
        Log.d(TAG, "addPlaces: ${place.name}")
...
private fun showInfoWindow(place: Place) {
    // Show in AR
    /* val matchingPlaceNode = ?.children?.filter {
       it is PlaceNode
     }?.first {
...
fun showInfoWindow() {
    ...
    /*HelloArRenderer.parent?.children?.filter {
